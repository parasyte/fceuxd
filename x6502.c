...
static INLINE uint8 RdRAM(unsigned int A) //bbit edited: this was changed so cheat substituion would work
{
 return(_DB=ARead[A](A));
// return(_DB=RAM[A]);
}
...
void X6502_Reset(void)
{
 _IRQlow=FCEU_IQRESET;
  ...
  //xod's debuger uses the following function, so it can be deleted.
...
extern void (*DoPCM)(void);

static INLINE void X6502_RunDebug(int32 cycles)
{
         ...
         { 
          ...
          else if(_IRQlow&FCEU_IQNMI)
          {
           if(!_jammed)
           {
            ...
            _P|=I_FLAG;         
                // Can this possibly be correct?  It fixes some games...
            _PC=RdMem(0xFFFA); 
            _PC|=RdMem(0xFFFB)<<8;
            _IRQlow&=~FCEU_IQNMI;
           }
          }
          else //if(!(PPU[2]/*&PPU[0]*/&0x80)) //if(!irqdisablehack || (_IRQlow&(FCEU_IQEXT|FCEU_IQTEMP)))
          { 
...
void X6502_Run(int32 cycles)
{
	...
	//#if(defined(C80x86) && defined(__GNUC__)) //bbit edited: all lines referring to pbackus were commented out
...
//	 X.PC=pbackus;
	 if(_IRQlow)
	 {
	  ...
	  { 
	   if(!(_PI&I_FLAG) && !_jammed)
	   {
	    ...
	    PUSH((_P&~B_FLAG)|(U_FLAG));
	    _P|=I_FLAG;
		if(loggingcodedata)LogCDVectors(2);
	    _PC=RdMem(0xFFFE);
	    _PC|=RdMem(0xFFFF)<<8;
	   }
	  }
	...
	if (numWPs | step | stepout | watchpoint[64].flags) breakpoint(); //bbit edited: this line added
...
//{
 //if(!debugmode)
  //X6502_RunNormal(cycles);
 //else
 // X6502_RunDebug(cycles);
//}
