...
void FASTAPASS(3) GINLINE setprg2r(int r, unsigned int A, unsigned int V)
{
  ...
  setpageptr(2,A,PRGptr[r]?(&PRGptr[r][V<<11]):0);
}
...
{
  V&=PRGmask4[r];
  setpageptr(4,A,PRGptr[r]?(&PRGptr[r][V<<12]):0);
}
...
void FASTAPASS(3) GINLINE setprg8r(int r, unsigned int A, unsigned int V)
{
  ...
  {
   V&=PRGmask8[r];
   setpageptr(8,A,PRGptr[r]?(&PRGptr[r][V<<13]):0);
  }
  else
  {
   uint32 VA=V<<2;
   int x;
   for(x=0;x<4;x++)
    setpageptr(2,A+(x<<11),PRGptr[r]?(&PRGptr[r][((VA+x)&PRGmask2[r])<<11]):0);
  }
}
...
void FASTAPASS(3) GINLINE setprg16r(int r, unsigned int A, unsigned int V)
{
  ...
  {
   V&=PRGmask16[r];
   setpageptr(16,A,PRGptr[r]?(&PRGptr[r][V<<14]):0);
  }
  else
  {
   uint32 VA=V<<3;
...
void FASTAPASS(3) GINLINE setprg32r(int r,unsigned int A, unsigned int V)
{
  ...
  {
   V&=PRGmask32[r];
   setpageptr(32,A,PRGptr[r]?(&PRGptr[r][V<<15]):0);
  }
  else
  {
   uint32 VA=V<<4;
