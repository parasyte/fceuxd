...
int BlockingCheck(void); //bbit edited: line added to eliminate compiler warning
...
static int discallb(uint16 a, char *s)
{
 SendDlgItemMessage(dwin,100,LB_ADDSTRING,0,(LPARAM)(LPSTR)s);
 if(a==Xsave->PC)
  SendDlgItemMessage(dwin,100,LB_SETCURSEL,discount,0);
 asavers[discount]=a;
 discount++;
 return((discount>25)?0:1);
}
...
// }
//} //bbit edited: commented out to remove compiler warnings
...
void BeginDSeq(HWND hParent)
{
 ...
 dwin=CreateDialog(fceu_hInstance,"XODDEBUGGER",0,DebugCon);
}
...
void XodUpdateDebugger(void)//bbit Edited: Changed to prevent conflicts with a function of the same name
{
 if(mwin)
  UpdateDMem(cmsi);
}

void XodKillDebugger(void) //bbit Edited: Changed to prevent conflicts with a function of the same name
{
 if(mwin)
  DestroyWindow(mwin);
 if(dwin)
