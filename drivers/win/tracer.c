	...
	switch(uMsg) {
		case WM_INITDIALOG:
			...
			if(logging_options == -1){
				logging_options = (LOG_REGISTERS | LOG_INSTRUCTION_DATA);
				...
				if(logging_options&LOG_INSTRUCTION_DATA)CheckDlgButton(hwndDlg, 111, BST_CHECKED);				
			}
				...
				case BN_CLICKED:
					switch(LOWORD(wParam)) {
						...
						case 110:
							...
							break;
						case 111:
							logging_options ^= LOG_INSTRUCTION_DATA;
							break; 
						case 114:
						...
						case 116:
							...
							log_update_window ^= 1;
							if((!userpause) && !log_update_window)
		...
		switch(uMsg) {
			case WM_VSCROLL:
				if (lParam) {
					...
					if((!userpause) && !log_update_window)break;
					...
					if((tracesi.nPos+tracesi.nPage) > tracesi.nMax) tracesi.nPos = tracesi.nMax-tracesi.nPage;
...
void BeginLoggingSequence(void){
	...
	if(logtofile){
		...
		fprintf(LOG_FP,"FCEUXD "FCEUXD_VERSION_STRING" - Trace Log File\n");
	} else {
		...
		tracelogbuf = malloc(j*sizeof(char *));
		for(i = 0;i < j;i++){
			tracelogbuf[i] = malloc(80); //todo: add out of memory error checking here
			tracelogbuf[i][0] = 0;
		}
...
void LogInstruction(){
	char address[7], data[11], disassembly[28], axystate[16];
	char str[96];
	int addr=X.PC;
	int size, j;
	//uint16 A=0;
...
/*
		if((oldcodecount != codecount) || 
			((olddatacount != datacount))){
				if(hMemView && (EditingMode == 2))UpdateColorTable();
				if(!logging){
					oldcodecount = codecount;
					olddatacount = datacount;
					return;
...
*/
	...
	if(logging_options & LOG_NEW_INSTRUCTIONS){
		if((oldcodecount != codecount) || 
			((olddatacount != datacount) && (logging_options & LOG_NEW_DATA))){ //something new was logged
			oldcodecount = codecount;
			olddatacount = datacount;
			if(unloggedlines > 0){
...
/*
	if(logging_options & LOG_SKIP_UNMAPPED){
		j = GetPRGAddress(addr);
		if (j != -1){
			k=0; //todo: clean this all up, make that for loop copy to the opcode array somewhere else
			if(cdloggerdata[j] != 0)k = 1;
			else {
				opcode[0] = GetMem(addr);
				for (i = 1; i < opsize[opcode[0]]; i++) opcode[i] = GetMem(addr+i);
				switch (optype[opcode[0]]) {
					...
					case 4: A = (GetMem(opcode[1]) | (GetMem(opcode[1]+1))<<8)+X.Y; break;
					case 5: A = opcode[1]+X.X; break;
					case 6: A = (opcode[1] | opcode[2]<<8)+X.Y; break;
					case 7: A = (opcode[1] | opcode[2]<<8)+X.X; break;
					case 8: A = opcode[1]+X.Y; break;
				}
				if((j = GetPRGAddress(addr)) != -1)
				if(cdloggerdata[GetPRGAddress(j)] != 0)k = 1;
			}
			if(k){
				if(logging_options & LOG_ADD_PERIODS){
					unloggedlines++;
				}
				return;
			}
		...
		if((unloggedlines > 0) && (logging_options & LOG_ADD_PERIODS)){
				sprintf(str,"... (%d lines skipped)",unloggedlines);
				OutputLogLine(str);
				unloggedlines = 0;
		}
...
void ShowLogDirDialog(void){
 ...
 if(nameo[0]){
	...
	logfilename = malloc(strlen(nameo)+1);
